 ...
 * 
 * Si le formulaire est correctement rempli : INSERT en bdd et redirection vers
 * la page de liste avec message de confirmation
 * Sinon on affiche les messages d'erreurs et champs pré-remplis avec les 
 * valeurs saisies
 * 
 * Adapter la page pour la modification
 * - si on reçoit un id dans l'url sans retour de POST, on prérempli le formulaire 
 * à partir de la bdd
 * - enregistrement avec UPDATE au lieu d'INSERT
 * adapter le contrôle de l'unicité de la référence pour exclure la référence du
 * produit que l'on modifie de la requête
 */
...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES['photo']['tmp_name'])) {
        // le poids du fichier en octet est plus lourd qu'1Mo
        if ($_FILES['photo']['size'] > 1000000) {
            $errors[] = 'La photo ne doit pas faire plus de 1Mo';
        }
        // les formats qu'on accepte
        $allowedMimeTypes = [
            'image/jpeg',
            'image/gif',
            'image/png'
...
<!--
    L'attribut enctype est obligatoire pour un formulaire qui contient un 
